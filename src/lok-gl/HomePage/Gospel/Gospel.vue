  ...
  <O3D v-if="screen">
    <O3D :pz="depth" :py="-screen.height * (scroller.maxY - scroller.value)" :px="screen.width * (animator.value + hider.value)">
      <O3D :layout="'open-menu'">
        <TextureText :text="'Gospel'" @remove="$removeClick($event)" @add="$addClick($event, () => { $emit('overlay', 'gospel') })" :align="'left'" :sdk="sdk" :base="base" :font="'SeasideResortNF'" :texture="'purple2DTexture'"></TextureText>
      </O3D>
    </O3D>
