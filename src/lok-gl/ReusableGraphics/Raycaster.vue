...
export default {
  ...
  async mounted () {
    const renderer = lookUp(this.$parent, 'renderer')
    // const renderer = await this.base.waitKN('renderer')
    ...
    const onDocumentClick = () => {
      ...
      if (bs.camera && bs.mouse && rc) {
        ...
        const event = (findings.length) > 0 ? findings[0] : null
        // console.log(first)
        if (event) {
          const obj = event.object
          const userData = obj.userData
          const handler = userData.handler
          if (handler) {
            handler({
              type: 'click',
              event,
